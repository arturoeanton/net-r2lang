...
public class Parser
{
    ...
    private INode parseImportStatement()
    {
        ...
        // consume ; si exist
        if (_curToken.Type == TokenType.SYMBOL && _curToken.Value == ";") NextToken();
    ...
    private INode parseLetStatement()
    {
        ...
        if (_curToken.Type == TokenType.IN) return new LetStatement { Name = name, Value = null };
    ...
    private INode parseForStatement()
    {
        ...
        if (_curToken.Type == TokenType.LET || _curToken.Type == TokenType.VAR)
        {
            ...
            if (_curToken.Type == TokenType.IN && _curToken.Value == "in")
            {
                ...
                INode initStmt = null;
                if (exp is Identifier) inArray = ((Identifier)exp).Name;
                if (exp is AccessExpression) inArray = ((AccessExpression)exp).Member;
        ...
        if (_curToken.Type != TokenType.SYMBOL || _curToken.Value != ";") condition = parseExpression();
        if (!(_curToken.Type == TokenType.SYMBOL && _curToken.Value == ";"))
            except("Falta ';' tras cond en for(...)");
        NextToken(); // comer ;

        INode post = null;
        if (!(_curToken.Type == TokenType.SYMBOL && _curToken.Value == ")")) post = parseAssignmentOrExprStatement();
        if (!(_curToken.Type == TokenType.SYMBOL && _curToken.Value == ")"))
    ...
    {
        var isClass = _curToken.Type == TokenType.CLASS;
        ...
        while (!(_curToken.Type == TokenType.SYMBOL && _curToken.Value == "}")
            ...
            else if (_curToken.Type == TokenType.FUNC || _curToken.Type == TokenType.FUNCTION ||
                     _curToken.Type == TokenType.METHOD)
    ...
    private INode parseExpression()
    {
        ...
        // parse binarios
        while (_curToken.Type == TokenType.SYMBOL &&
               (IsBinaryOp(_curToken.Value) || _curToken.Value == "!" || _curToken.Value == "="))
        {
            var op = _curToken.Value;

            if (_peekToken.Type == TokenType.SYMBOL && _peekToken.Value == "=")
            {
                NextToken();
                op += "=";
    ...
    private INode parseFactor()
    {
        ...
        if (_curToken.Type == TokenType.FUNC || _curToken.Type == TokenType.FUNCTION) return parseAnonymousFunction();
        // number
        if (_curToken.Type == TokenType.NUMBER)
        {
            double.TryParse(_curToken.Value, out var num);
        ...
        // array => [...]
        if (_curToken.Type == TokenType.SYMBOL && _curToken.Value == "[") return parseArrayLiteral();
        // map => { key: val, ... }
        if (_curToken.Type == TokenType.SYMBOL && _curToken.Value == "{") return parseMapLiteral();
    ...
    private List<string> parseFunctionArgs()
    {
               ...
               && _curToken.Type != TokenType.EOF)
        {
            ...
            if (_curToken.Type == TokenType.SYMBOL && _curToken.Value == ")") break;
            NextToken();
        }
